...
function initializeApp() {
  ...
  // Set Indian Ocean as default filter and update UI
  appState.filters.region = "Indian Ocean";
  const regionFilter = document.getElementById("regionFilter");
  if (regionFilter) regionFilter.value = "Indian Ocean";
}
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", toggleFilterDrawer);
  document
...
function generateArgoFloats() {
  ...
  const centerLon = 90; // Indian Ocean longitude
  const centerLat = 10; // Indian Ocean latitude

  appState.argoFloats = mockArgoData.map((float) => {
    // Convert lat/lon to globe coordinates
    const x = centerX + ((float.lon - centerLon) / 180) * radius * 0.8;
    const y = centerY - ((float.lat - centerLat) / 90) * radius * 0.8;
...
function selectArgoFloat(e) {
  ...
  openChatOverlay();
  addOverlayMessage(
    `Show me data for ARGO float ${dot.dataset.floatId} in ${region} from ${year}`
  );
}
...
function addChatbotResponse(userMessage) {
  ...
  anime({
    ...
    update: function (anim) {
      messageText.textContent = response.substring(
        0,
        Math.floor((anim.progress * response.length) / 100)
      );
    },
  });
}
...
function generateMockResponse(userMessage) {
  const responses = {
    temperature:
      "Based on recent ARGO float data, surface temperatures in the Indian Ocean average 28.5°C, with seasonal variations of ±2°C. The Bay of Bengal shows higher temperatures (29-30°C) compared to the Arabian Sea (27-28°C).",
    salinity:
      "Salinity levels in the Indian Ocean range from 34.5-35.5 PSU. The Arabian Sea shows higher salinity (35.2 PSU) due to evaporation, while the Bay of Bengal has lower salinity (34.8 PSU) due to freshwater input from rivers.",
    ...
    default:
      "I can help you explore ocean data from ARGO floats. You can ask about temperature, salinity, ocean currents, or specific regions. Try asking about conditions near your location or a specific ocean parameter.",
  };
  ...
  switch (view) {
    case "satellite":
      globe.style.background =
        "radial-gradient(circle at 30% 30%, #1e40af, #1e3a8a, #1e293b)";
      break;
    case "terrain":
      globe.style.background =
        "radial-gradient(circle at 30% 30%, #059669, #047857, #065f46)";
      break;
    case "political":
      globe.style.background =
        "radial-gradient(circle at 30% 30%, #7c3aed, #6d28d9, #5b21b6)";
      break;
    case "night":
      globe.style.background =
        "radial-gradient(circle at 30% 30%, #1f2937, #111827, #0f172a)";
      break;
  }
...
function confirmExport() {
  ...
  setTimeout(() => {
    ...
    showNotification(
      `Data exported successfully as ${fileName}.${format}`,
      "success"
    );
  }, 2000);
}
