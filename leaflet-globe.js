...
class LeafletGlobe {
  ...
  async init() {
    try {
      ...
      this.map = L.map("leafletMap", {
        center: [10, 320], // Equator & prime meridian
  ...
  addEsriImageryLayer() {
    ...
    const esriLayer = L.tileLayer(
      ...
      {
        attribution:
          "Tiles © Esri, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community",
        maxZoom: 18,
        minZoom: 1,
      }
    ).addTo(this.map);
  ...
  generateMockArgoData() {
    ...
    regions.forEach((region) => {
      ...
      for (let i = 0; i < numFloats; i++) {
        const lat =
          region.bounds[0][0] +
          Math.random() * (region.bounds[1][0] - region.bounds[0][0]);
        const lng =
          region.bounds[0][1] +
          Math.random() * (region.bounds[1][1] - region.bounds[0][1]);
        ...
        argoData.push({
          ...
          parameters: ["temperature", "salinity", "pressure", "oxygen"].slice(
            0,
  ...
  createArgoMarker(float) {
    ...
    const popupContent = `
            <div class="argo-popup">
                ...
                <p><strong>Quality:</strong> <span class="quality-${
                  float.quality
                }">${float.quality}</span></p>
                <p><strong>Last Update:</strong> ${float.lastUpdate.toLocaleDateString()}</p>
                <p><strong>Parameters:</strong> ${float.parameters.join(
                  ", "
                )}</p>
                <div class="popup-actions">
                    <button class="btn-primary" onclick="window.leafletGlobe.selectArgoFloat('${
                      float.id
                    }')">
                        <i class="fas fa-eye"></i> View Details
                    </button>
                    <button class="btn-secondary" onclick="window.leafletGlobe.askAboutFloat('${
                      float.id
                    }')">
  ...
  getIconColor(float) {
    ...
    switch (float.quality) {
      ...
      case "medium":
        return "#f59e0b"; // Yellow for medium quality
      case "low":
  ...
  selectArgoFloat(floatId) {
    ...
    if (float) {
      ...
      const marker = this.argoMarkers.find(
        (m) => m.options.floatId === floatId
      );
      if (marker) {
        marker.openPopup();
      }
